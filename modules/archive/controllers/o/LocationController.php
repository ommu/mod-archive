...
class LocationController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = ArchiveLocation::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('ArchiveLocation', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Archive Locations');
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['ArchiveLocation'])) {
			$model->attributes=$_POST['ArchiveLocation'];
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'ArchiveLocation success created.'));
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['ArchiveLocation'])) {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'ArchiveLocation success updated.'));
					//$this->redirect(array('view','id'=>$model->location_id));
					$this->redirect(array('manage'));
				}
			}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				ArchiveLocation::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				ArchiveLocation::model()->deleteAll($criteria);
			}
		}
	...
	{
		$model = ArchiveLocation::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='archive-location-form') {

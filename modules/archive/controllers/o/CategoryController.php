...
class CategoryController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = ArchiveConvertCategory::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('ArchiveConvertCategory', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Archive Convert Categories');
	...
	{
		$model=new ArchiveConvertCategory('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['ArchiveConvertCategory'])) {
			$model->attributes=$_GET['ArchiveConvertCategory'];
		}
	...
	{
		$model=new ArchiveConvertCategory;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['ArchiveConvertCategory'])) {
			$model->attributes=$_POST['ArchiveConvertCategory'];
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'ArchiveConvertCategory success created.'));
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['ArchiveConvertCategory'])) {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'ArchiveConvertCategory success updated.'));
					//$this->redirect(array('view','id'=>$model->category_id));
					$this->redirect(array('manage'));
				}
			}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				ArchiveConvertCategory::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				ArchiveConvertCategory::model()->deleteAll($criteria);
			}
		}
	...
	{
		$model = ArchiveConvertCategory::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='archive-convert-category-form') {

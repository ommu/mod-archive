...
if (!$small) {
    ...
    $this->params['breadcrumbs'][] = ['label' => $model->archive->isFond ? $model->archive->code : Yii::t('app', '{level-name} {code}', ['level-name' => $model->archive->levelTitle->message, 'code' => $model->archive->code]), 'url' => [($model->archive->isFond ? 'fond' : 'admin').'/view', 'id' => $model->archive_id]];
